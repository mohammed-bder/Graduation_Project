...
namespace Graduation_Project.Api.Controllers.Shared
{
    ...
    //    [HttpGet("available-slotss/{doctorId:int}")]
    //    public async Task<IActionResult> GetAvailableSlotss(int doctorId, int slotDurationMinutes, DateOnly selectedWeekStart)
    //    {
    //        try
    //        {
    //            var spec = new WorkSheduleSpecifications(doctorId);
    ...
    //            var appointmentSpec = new AppointmentSpecifications(doctorId, selectedWeekStart);
    //            var bookedAppointments = await _unitOfWork.Repository<Appointment>().GetAllWithSpecAsync(appointmentSpec);
